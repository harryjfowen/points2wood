...
class PointNetConv(MessagePassing):
    ...
    def message(self, x_j: Optional[Tensor], pos_i: Tensor,
            ...
            #msg[:, :3] = (pos_j[:, :3] - pos_i[:, :3]) / self.radius
            ...
            # if torch.sum(pos_j[:, 3]) != 0:
            #     mean_reflectance = scatter_mean(pos_j[:, 3], edge_index_i, dim=0)[edge_index_i]
            #     #std_reflectance = scatter_std(pos_j[:, 3], edge_index_i, dim=0)[edge_index_i]
            #     #mean_reflectance = mean_reflectance * std_reflectance
            #     msg[:, 3] = mean_reflectance
            # else:
            #     msg[:, 3] = pos_j[:, 3]
